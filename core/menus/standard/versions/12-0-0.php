	...
	if ($mainmenu)	// If this is empty, loading hard coded menu and loading personalised menu will fail
	{
		...
		if ($mainmenu == 'commercial')
		{
			...
			if (!empty($conf->propal->enabled))
			{
				...
				// // FirstMenu
				// $newmenu->add("/comm/index.php?leftmenu=propals", $langs->trans("CommercialArea"), 0, (!empty($user->rights->propal->lire) || !empty($user->rights->commande->lire) || !empty($user->rights->fournisseur->lire) || !empty($user->rights->supplier_proposal->lire) || !empty($user->rights->supplier_order->lire) || !empty($user->rights->contrat->lire) || !empty($user->rights->ficheinter->lire) ), '', $mainmenu, 'propals', 100);
		...
		if ($mainmenu == 'billing')
		{
			...
			if (!empty($conf->facture->enabled))
			{
				...
				// // FirstMenu
				// $newmenu->add("/compta/index.php?leftmenu=customers_bills", $langs->trans("AccountancyTreasuryArea"), 0, (!empty($user->rights->facture->lire) || !empty($user->rights->don->contact->lire)
	   //      || !empty($user->rights->tax->charges->lire) || !empty($user->rights->salaries->read)
	   //      || !empty($user->rights->fournisseur->facture->lire) || !empty($user->rights->loan->read) || !empty($user->rights->margins->liretous)), '', $mainmenu, 'customers_bills');
		...
		if ($mainmenu == 'accountancy')
		{
            ...
            if (!empty($conf->comptabilite->enabled))
            {
				// // FirstMenu
				// $langs->load('accountancy');
				// $newmenu->add("/accountancy/index.php?mainmenu=accountancy&amp;leftmenu=accountancy_files", $langs->trans("AccountancyArea"), 0, (!empty($user->rights->compta->resultat->lire) || !empty($user->rights->accounting->mouvements->lire) || !empty($user->rights->asset->read)), '', $mainmenu, 'files');
            	// Files
            	if (empty($conf->global->ACCOUNTANCY_HIDE_EXPORT_FILES_MENU))
            	{
		...
		if ($mainmenu == 'mrp')
		{
		    ...
		    if (!empty($conf->bom->enabled) || !empty($conf->mrp->enabled))
		    {
		        ...
		        $newmenu->add("/mrp/index.php", $langs->trans("MenuBOM"), 0, $user->rights->bom->read, '', $mainmenu, 'bom');
		    ...
		    if (!empty($conf->mrp->enabled))
		    {
		    	...
		    	$newmenu->add("/mrp/index.php", $langs->trans("MenuMRP"), 0, $user->rights->mrp->read, '', $mainmenu, 'mo');
		...
		*/
		if ($mainmenu == 'hrm')
		{
			...
			if (!empty($conf->holiday->enabled))
			{
				...
				// // FirstMenu
				// $newmenu->add("/hrm/index.php?mainmenu=hrm&leftmenu=hrm", $langs->trans("HRMArea"), 0, (!empty($user->rights->hrm->employee->read) || !empty($user->rights->holiday->write) || !empty($user->rights->deplacement->lire) || !empty($user->rights->expensereport->lire)), '', $mainmenu, 'hrm');
		...
		if ($mainmenu == 'tools')
		{
			if (empty($user->socid)) // limit to internal users
			{
				$langs->load("mails");
				// // FirstMenu
				// $newmenu->add("/core/tools.php?leftmenu=email_templates", $langs->trans("Tools"), 0, 1, '', $mainmenu, 'email_templates');

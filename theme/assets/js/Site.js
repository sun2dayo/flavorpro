  ...
  function (_Base) {
    ...
    babelHelpers.createClass(Site, [{
      ...
      value: function menubarType(type) {
        var typeds = type;
        if(typeds !== 'fold' && typeds !== 'unfold')
          typeds = 'fold';

        window.localStorage.setItem('ds_rp_menu', typeds);
        if(typeds == 'fold'){
          $('.mm-panels .mm-panel.mm-hasnavbar .mm-navbar > a').addClass('animateddsmenu');
        }else{
          $('.mm-panels .mm-panel.mm-hasnavbar .mm-navbar > a').removeClass('animateddsmenu');
        }

    ...
    if (document.addEventListener) {
      document.addEventListener('DOMContentLoaded', function () {
        ...
        var Skintools = {
          ...
          tpl: '<div class="site-skintools">' + '<div class="site-skintools-inner">' + '<div class="site-skintools-toggle">' + '<i class="icon md-settings primary-600"></i>' + '</div>' + '<div class="site-skintools-content">' + '<div class="nav-tabs-horizontal">' + '<ul role="tablist" class="nav nav-tabs nav-tabs-line">' + '<li class="nav-item"><a class="nav-link active" role="tab" aria-controls="skintoolsSidebar" href="#skintoolsSidebar" data-toggle="tab" aria-expanded="true">Sidebar</a></li>' + '<li class="nav-item"><a class="nav-link" role="tab" aria-controls="skintoolsNavbar" href="#skintoolsNavbar" data-toggle="tab" aria-expanded="false">Navbar</a></li>'  + '</ul>' + '<div class="tab-content">' + '<div role="tabpanel" id="skintoolsSidebar" class="tab-pane active"></div>' + '<div role="tabpanel" id="skintoolsNavbar" class="tab-pane"></div>' + '<button class="btn btn-block btn-primary mt-20" id="skintoolsReset" type="button">Reset</button><span id="revolonlyfordemo">Only For Demo</span>' + '</div>' + '</div>' + '</div>' + '</div>' + '</div>',
          skintoolsSidebar: ['dark', 'light'],
          skintoolsNavbar: ['primary', 'blue', 'brown', 'cyan', 'green', 'grey', 'orange', 'pink', 'purple', 'red', 'teal', 'yellow'],
          navbarSkins: 'bg-primary-600 bg-blue-600 bg-brown-600 bg-cyan-600 bg-green-600 bg-grey-600 bg-orange-600 bg-pink-600 bg-purple-600 bg-red-600 bg-teal-600 bg-yellow-800',
          ...
          init: function init() {
            ...
            $('#skintoolsSidebar input').on('click', function () {
              $('li#ordernowbuttoninmenu>a').remove();
          ...
          navbarImprove: function navbarImprove(val, checked) {
            if (val === 'inverse') {
              ...
              $("body").removeClass (function (index, className) {
                  return (className.match (/(^|\s)demobg-\S+/g) || []).join(' ');
              });
              $('body').addClass('onlyfordemo demo'+bg);
            }
          },
          ...
          reset: function reset() {
            localStorage.clear();
            this.initLocalStorage();
            this.$demobody.removeClass('onlyfordemo');
          }
        };
        Skintools.init();
      });
